...
// ===== PLAY STORE V3 - ALL APPS ALWAYS VISIBLE =====

// Show all games in store (installed or not)
function renderPlayStoreStore() {
    const grid = document.getElementById('ps-store-grid');
    if (!grid) return;
    ...
    // Show ALL games from PS_GAMES database
    Object.values(PS_GAMES).forEach(game => {
        const isInstalled = installedIds.includes(game.id);
        html += `<div class="ps-store-card" data-category="${game.category}" onclick="openPlayStoreDetail('${game.id}', 'game')">
            <div class="card-icon">${game.icon}</div>
            ...
            <div class="install-overlay">
                ${isInstalled 
                    ? `<button onclick="event.stopPropagation(); openApp('${game.id}')">▶ Play Now</button>`
                    : `<button onclick="event.stopPropagation(); installPlayStoreGame('${game.id}', '${game.icon}', '${game.name}')">⬇ Install</button>`
                }
            </div>
        </div>`;
...
// Show all apps in store (installed or not)
function renderPlayStoreApps() {
    const grid = document.getElementById('ps-apps-grid');
    if (!grid) return;
    ...
    // Show ALL apps from PS_APPS database
    Object.values(PS_APPS).forEach(app => {
        const isInstalled = installedIds.includes(app.id);
        html += `<div class="ps-app-card" onclick="openPlayStoreDetail('${app.id}', 'app')">
            <div class="card-icon">${app.icon}</div>
            <div class="card-title">${app.name}</div>
            <div class="card-meta">${app.category} ${isInstalled ? '✓ Installed' : ''}</div>
            <div class="install-overlay" style="position: absolute; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0,0,0,0.7); border-radius: 12px; display: none; align-items: center; justify-content: center; flex-direction: column; gap: 8px; backdrop-filter: blur(4px);">
                ${isInstalled 
                    ? `<button onclick="event.stopPropagation(); openApp('${app.id}')" style="background: var(--ps-primary); color: white; border: none; padding: 8px 20px; border-radius: 20px; font-size: 12px; font-weight: 600; cursor: pointer; font-family: inherit; transition: 0.2s;">▶ Open</button>`
                    : `<button onclick="event.stopPropagation(); installPlayStoreItem('${app.id}', '${app.icon}', '${app.name}', 'app')" style="background: var(--ps-primary); color: white; border: none; padding: 8px 20px; border-radius: 20px; font-size: 12px; font-weight: 600; cursor: pointer; font-family: inherit; transition: 0.2s;">⬇ Install</button>`
                }
            </div>
        </div>`;
    });
    ...
    // Add hover effect for install overlay
    setTimeout(() => {
        document.querySelectorAll('.ps-app-card').forEach(card => {
            card.addEventListener('mouseenter', function() {
                const overlay = this.querySelector('.install-overlay');
...
// Detail view for any game/app (works whether installed or not)
function openPlayStoreDetail(itemId, type) {
    const overlay = document.getElementById('ps-detail-overlay');
    const content = document.getElementById('ps-detail-content');
    if (!overlay || !content) return;
        ...
        <div style="display: flex; align-items: flex-start; gap: 20px; margin-bottom: 20px;">
            ...
            <div style="flex: 1;">
                ...
                <div class="ps-detail-meta">
                    ...
                    <span>${item.category.charAt(0).toUpperCase() + item.category.slice(1)}</span>
                    <span>${isGame ? '🎮 Game' : '📱 App'}</span>
                    ${isInstalled ? '<span style="background: rgba(52, 168, 83, 0.3); color: #81c995;">✓ Installed</span>' : ''}
                </div>
                <div class="ps-detail-actions">
                    ${isInstalled 
                        ? `<button class="ps-detail-btn play" onclick="openApp('${itemId}')">▶ Play Now</button>`
                        : `<button class="ps-detail-btn install" onclick="installPlayStoreItem('${itemId}', '${item.icon}', '${item.name}', '${type}')">⬇ Install</button>`
                    }
                    <button class="ps-detail-btn install" onclick="toggleFavorite('${itemId}')">⭐ Favorite</button>
                </div>
...
// Install from detail view - adds to launcher and taskbar
function installPlayStoreItem(appId, icon, name, type) {
    const launcherList = document.getElementById('launcher-list');
    const existingItem = launcherList ? launcherList.querySelector(`[data-app-id="${appId}"]`) : null;
    if (existingItem) {
    ...
    setTimeout(() => {
        ...
        notificationMgr.showNotification({ title: "Installed!", message: `${name} has been added to your launcher. Click to play!`, icon: "sparkles" });
        ...
        // If detail view is open, refresh it to show "Play Now" button
        const overlay = document.getElementById('ps-detail-overlay');
        if (overlay && overlay.style.transform === 'translateX(0px)') {
            openPlayStoreDetail(appId, type);
        }
...
// Override renderPlayStoreHome to show recommendations when no recent games
const originalRenderPlayStoreHome = renderPlayStoreHome;
renderPlayStoreHome = function() {
    const recent = getRecentPlays();
    const emptyDiv = document.getElementById('ps-home-empty');
    ...
    // Always show content - either recent games or featured recommendations
    if (emptyDiv) emptyDiv.style.display = 'none';
    if (contentDiv) contentDiv.style.display = 'block';

    // Show featured = most recent played, or first game if none
    const featuredId = recent.length > 0 ? recent[0] : Object.keys(PS_GAMES)[0];
    renderFeaturedGame(featuredId);

    // Show recent grid - mix of recent plays and recommended games
    if (recentSection && recentGrid) {
        recentSection.style.display = 'block';
        recentSection.querySelector('h3').innerText = recent.length > 0 ? 'Recent Play' : 'Recommended for You';

        let displayIds = recent.length > 0 ? recent.slice(1, 5) : Object.keys(PS_GAMES).slice(1, 5);

        recentGrid.innerHTML = displayIds.map(appId => {
            const game = PS_GAMES[appId];
            if (!game) return '';
            return `<div class="ps-store-card" onclick="openPlayStoreDetail('${appId}', 'game')">
                <div class="card-icon">${game.icon}</div>
...
// Override renderPlayStoreLibrary to show installed items with browse button
const originalRenderPlayStoreLibrary = renderPlayStoreLibrary;
renderPlayStoreLibrary = function() {
    const grid = document.getElementById('ps-library-grid');
    const empty = document.getElementById('ps-library-empty');
    ...
    if (installed.length === 0) {
        ...
        if (empty) {
            ...
            empty.innerHTML = `
                ...
                <h3 style="margin: 0 0 10px; font-weight: 500;">Library Empty</h3>
                <p style="margin: 0; font-size: 13px;">Install apps and games from the Store to see them here.</p>
                <button onclick="showPlayStoreTab('store')" style="margin-top: 15px; background: var(--ps-primary); color: white; border: none; padding: 10px 20px; border-radius: 20px; cursor: pointer; font-weight: 500; font-family: inherit;">Browse Store</button>
            `;
        }
        return;
    }
...
// Override renderPlayStoreSidebar to show all installed items with browse buttons
const originalRenderPlayStoreSidebar = renderPlayStoreSidebar;
renderPlayStoreSidebar = function() {
    const list = document.getElementById('ps-game-list');
    if (!list) return;
    ...
    if (psCurrentCategory === 'games') {
        ...
        if (!installed.find(a => PS_GAMES[a.id])) {
            html = '<div style="padding: 20px; text-align: center; color: #9aa0a6; font-size: 12px;">No games installed<br><button onclick="showPlayStoreTab('store')" style="margin-top: 10px; background: rgba(74,158,255,0.2); color: #4a9eff; border: none; padding: 6px 12px; border-radius: 10px; cursor: pointer; font-size: 11px; font-family: inherit;">Browse Games</button></div>';
        }
    } else {
        // Show installed apps in sidebar
        installed.forEach(app => {
        ...
        if (!installed.find(a => PS_APPS[a.id])) {
            html = '<div style="padding: 20px; text-align: center; color: #9aa0a6; font-size: 12px;">No apps installed<br><button onclick="switchPlayStoreCategory('apps')" style="margin-top: 10px; background: rgba(74,158,255,0.2); color: #4a9eff; border: none; padding: 6px 12px; border-radius: 10px; cursor: pointer; font-size: 11px; font-family: inherit;">Browse Apps</button></div>';
        }
    }
...
// Update the init when store opens to preload all tabs
const originalOpenAppPS_v3 = openApp;
openApp = function(appId) {
    const result = originalOpenAppPS_v3(appId);
    if (appId === 'store-window') {
        setTimeout(() => {
            // Set username
            const usernameEl = document.getElementById('ps-username');
            ...
            renderPlayStoreSidebar();
            renderPlayStoreHome();
            renderPlayStoreStore(); // Pre-load store tab
            renderPlayStoreApps(); // Pre-load apps tab
        }, 100);
    }
    return result;
};
